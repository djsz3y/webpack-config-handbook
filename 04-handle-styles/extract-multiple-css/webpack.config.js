...
module.exports = {
  ...
  plugins: [
    ...
    // plugins 里 ExtractTextPlugin 插件的 [name]@[contentHash:8].css
    // 和 output.filename 的 [name].js
    // 中的 name，
    // 都是指代 chunk 的名字，即 entry 中我们为每一个入口分配的名字（foo、bar）。
    // 这里可以验证以下打包结果，如图 5-2 所示。
    // 和预期相符，[name] 指代的是 chunk 的名字，Asset 与 Chunk Names 是相对应的。
  ],
  devServer: {
    publicPath: "/dist/",
    port: 3000,
